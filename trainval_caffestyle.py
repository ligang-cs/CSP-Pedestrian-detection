...
config = Config()
config.train_path = '/ligang/Dataset/citypersons'
config.test_path = '/ligang/Dataset/citypersons'
config.gpu_ids = [0, 1]
config.onegpu = 4
...
if config.val:
    testdataset = CityPersons(path=config.train_path, type='val', config=config,
                              caffemodel=config.caffemodel, preloaded=False)
    testloader = DataLoader(testdataset, batch_size=1)

# net
print('Net...')
net = CSPNet().cuda()
# This is important since the pretrained model is different under config.caffemodel == True
pretrained_dict = torch.load('./resnet101_caffe.pth')
