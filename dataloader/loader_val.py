...
class CityPersons(Dataset):
    ...
    def __getitem__(self, item):
        if self.caffemodel:
            ...
            if self.type == 'train':
                ...
                gts, igs = self.dataset[item]['bboxes'], self.dataset[item]['ignoreareas']
                y_center, y_height, y_offset = self.calc_gt_center(gts, igs, radius=2, stride=self.config.down)
                ...
                return x_img, y_center, y_height
    ...
    def calc_gt_center(self, gts, igs, radius=2, stride=4):
        ...
        scale_map = np.zeros((2, int(self.config.size_test[0] / stride), int(self.config.size_test[1] / stride)))
        offset_map = np.zeros((3, int(self.config.size_test[0] / stride), int(self.config.size_test[1] / stride)))
        pos_map = np.zeros((3, int(self.config.size_test[0] / stride), int(self.config.size_test[1] / stride)))
